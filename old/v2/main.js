...
let ticker = 5;   //ticker time in minutes  default: 1,5,10
let enableOrders = false;//true;//false;    default: true
let quotes = ["BTC", "ETH", "BNB"];
let portf = [];         //array of currencies owned
let quote;
let exS = false;    //existence of fiat symbol
let alt;
let strategy = "PINGPONG"; // = "smaX";          //"emaX", "MMDiff", "upDown", "smaX", "macD"
let strategy2 = "UPDOWN";
                        ...
                        function setBots(sym) {
                            ...
                            function checkFiatSymbolExistence(s) {
                                for (i = 0; i <= syms.length; i++) {    //checks for symbol existence
                                    if (s == syms[i]) {
                                        exS = true;
                                        f.cs("Fiat symbol " + marks[0].market + " exist: " + exS);
                                        runBots();
                                    }
                                }
                            }
